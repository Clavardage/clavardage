 ...
 * -dans editMsg : -modifier couleur Hello... mais si on écrit dedans, le texte est quand même noir
...
=======
>>>>>>> 026a5473e625efdb20884932b3ef8b276a37ab17
	/* ** Menu Bar ** */
...
<<<<<<< HEAD
		...
		editMsg.addMouseListener( new  MouseAdapter(){
			public void mousePressed(MouseEvent e) {
				if (((MyRoundJTextField) editMsg).isEmpty()) {
					editMsg.setText("");
					editMsg.setForeground(COLOR_TEXT);
	...
	private void customThemeApp(ColorThemeApp c) {
		if (c == ColorThemeApp.LIGHT) {
			...
			COLOR_OUR_MESSAGE = new Color(212,212,212) ;
			COLOR_SHADOW = new Color(165,165,165,50) ;
			COLOR_TEXT = new Color(0,0,0);
			COLOR_TEXT_EDIT = new Color(127,127,127);
			...
			COLOR_OUR_MESSAGE = new Color(43,43,43) ;
			COLOR_SHADOW = new Color(165,165,165,50) ;
		...
		for (Component panel : listUsers.getComponents()) {
			((DestinataireJPanel) panel).setForegroundNamePanel(COLOR_TEXT);
		}
		...
		editMsg.setForeground(COLOR_TEXT_EDIT);
	}
...
>>>>>>> 026a5473e625efdb20884932b3ef8b276a37ab17
	...
	private void addGroupToList(String name, Boolean connect) throws IOException {
		...
		group.setForegroundNamePanel(COLOR_TEXT);
